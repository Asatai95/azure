...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        print(self.user)
        # self.user = await self.get_user_info(int(self.user))
        # print(self.user)
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            ...
            message_cm = text_data_json['message_cm']
            await self.channel_layer.group_send (
                self.room_group_name,
                {
                    'type': 'chat_message',
    ...
    async def chat_message(self, event):
        try :
            ...
            if message != "":
                cus = "customer"
                await self._save_message(message, cus)
            await self.send(text_data=json.dumps({
                'message': message,
            }))

        except:
            message_cm = event['message_cm']
            if message_cm != "":
                cm = "company"
                await self._save_message(message_cm, cm)
            await self.send(text_data=json.dumps({
                'message_cm': message_cm,
            }))
    ...
    def _save_message(self , message, user):
        message = message.replace(" u", "").replace(" c", "").replace("c", "").replace("u", "")
        print(message)
